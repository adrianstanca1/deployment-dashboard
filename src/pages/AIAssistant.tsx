...
import {
  ...
  Trash2, ChevronDown,
  Copy, Check, Wrench, Bug, Code, FileText, Settings,
  Cloud, Layers, Kanban,
  Brain, GitBranch,
  X, Download,
  Target, Command,
...
What would you like to work on today?`,
  ...
  // Read server state from shared React Query cache (populated by other pages)
  // staleTime ensures we use cached data rather than triggering new requests
  ...
  // Fetch AI providers — staleTime avoids re-fetching on every page visit
  const { data: providersData } = useQuery({
    queryKey: ['ai-providers-assistant'],
    queryFn: async () => {
  ...
  const switchProvider = async (providerId: string) => {
    try {
      ...
      // Invalidate both keys so header + AISettings stay in sync
      queryClient.invalidateQueries({ queryKey: ['ai-providers'] });
      queryClient.invalidateQueries({ queryKey: ['ai-providers-assistant'] });
    } catch (error) {
                  ...
                  {providersData?.map((provider) => (
                    ...
                    >
                      ...
                      <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded-full ${
                        ...
                        {provider.isActive ? 'Active' : provider.enabled ? 'Inactive' : 'Disabled'}
                      </span>
                    </button>
                  ))}
                </div>
