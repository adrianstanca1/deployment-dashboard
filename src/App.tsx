...
function AIStatusPill({ failingCount, onNavigate }: { failingCount: number; onNavigate: () => void }) {
  ...
  return (
    <button
      onClick={onNavigate}
      className={`flex items-center gap-2 px-4 py-1.5 border-b text-xs transition-colors w-full text-left ${
        isActive
          ? 'border-green-500/10 bg-transparent text-green-500/70 hover:text-green-400'
